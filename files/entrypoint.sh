...
# Exit immediately if a *pipeline* returns a non-zero status. (Add -x for
# command tracing)
set -e
...
case $1 in
  start)
    ...
    if [[ -n $CREATE_TOPICS ]]; then
      ...
      # Start a subshell in the background that waits for the Kafka broker to
      # open socket on port 9092 and then creates the topics when the broker is
      # running and able to receive connections ...
      (
